...
;********************PHA-Daten********************************************
tmn=1440.*tage
gesp4=dblarr(tmn)&prop4=dblarr(tmn)&gesp8=dblarr(tmn)&prop8=dblarr(tmn)
gesp25=dblarr(tmn)&prop25=dblarr(tmn)&geshe8=dblarr(tmn)&helhe8=dblarr(tmn)
geshe25=dblarr(tmn)&helhe25=dblarr(tmn);&helh34=dblarr(tmn)
geshe4=dblarr(tmn)&helhe4=dblarr(tmn)&gese150=dblarr(tmn)&ele150=dblarr(tmn)
gese300=dblarr(tmn)&ele300=dblarr(tmn)&gese1300=dblarr(tmn)
ele1300=dblarr(tmn)
;********************open phx-files*****************************************
;print,phxct," total number phx-files"
for opener = 0.,phxct-1. do begin
 ;print,opener  
 close,3 
 openr,3,filelistphx(opener),max(opener);, /GET_LUN
 var=dblarr(18)
 if EOF(3) ne 1 then readf,3,var else goto, pbad
 CLOSE, /ALL
 ON_IOERROR,NULL
 a=0 
 openr,3,filelistphx(opener)            ;***
 WHILE a EQ 0 DO BEGIN 
  ON_IOERROR,pbad
  readf,3,var
  ty=float(var(0))
  ...
  ;IF coinc EQ 3 OR coinc EQ 7 OR coinc EQ 11 THEN dtot=ade+bde+cde+dde+ede
  IF zraum LT 1440.*tage THEN BEGIN
   IF coinc EQ 0 THEN BEGIN
    gese150(zraum)=gese150(zraum)+1.
    IF dtot GT 0.25 AND dtot LT 0.7 THEN ele150(zraum)=ele150(zraum)+1.
   ENDIF
   IF coinc EQ 1 THEN BEGIN
    gese300(zraum)=gese300(zraum)+1.
    IF dtot GT 0.67 AND dtot LT 3.0 THEN ele300(zraum)=ele300(zraum)+1.
   ENDIF
   IF coinc EQ 2 THEN BEGIN
    gese1300(zraum)=gese1300(zraum)+1.
    IF dtot GT 2.64 AND dtot LT 10.4 THEN ele1300(zraum)=ele1300(zraum)+1.
   ENDIF
   IF coinc GT 2 THEN BEGIN
;******************particle species*****************************************
    ...
    ENDIF
;**************************************************************************
    ;arbitrary combination A B
    ;old boundaries: p4   1.604 - 1.9611   new:  1.73 - 1.92
    ;                p8   1.604 - 1.9611         1.73 - 1.92
    ;                p25  1.604 - 1.9611         1.65 - 1.94
    ;                h4   0.892 - 1.116          0.89 - 1.08
    ;                h8   0.892 - 1.116          0.91 - 1.08
    ;                h25  0.892 - 1.116          0.91 - 1.08 

    ;combination A0 B0 (ring off)
    ;old boundaries: p4   new:  1.81 - 1.90
    ;                p8         1.80 - 1.90
    ;                p25        1.70 - 1.92
    ;                h4         0.97 - 1.03
    ;                h8         0.97 - 1.03
    ...
    IF stat(zraum) GE 2 THEN BEGIN
     IF coinc EQ 4 THEN BEGIN
      gesp4(zraum)=gesp4(zraum)+1.
      IF teson GE 1.81 AND teson LT 1.90 THEN prop4(zraum)=prop4(zraum)+1.
     ENDIF
     IF coinc EQ 5 THEN BEGIN
      gesp8(zraum)=gesp8(zraum)+1.
      IF teson GE 1.80 AND teson LT 1.90 THEN prop8(zraum)=prop8(zraum)+1.
     ENDIF
     IF coinc EQ 6 THEN BEGIN
      gesp25(zraum)=gesp25(zraum)+1.
      IF teson GE 1.70 AND teson LT 1.92 THEN prop25(zraum)=prop25(zraum)+1.
     ENDIF
     IF coinc EQ 8 THEN BEGIN
      geshe4(zraum)=geshe4(zraum)+1.
      IF teson GE 0.97 AND teson LT 1.03 THEN helhe4(zraum)=helhe4(zraum)+1.
     ENDIF
     IF coinc EQ 9 THEN BEGIN
      geshe8(zraum)=geshe8(zraum)+1.
      IF teson GE 0.97 AND teson LT 1.03 THEN helhe8(zraum)=helhe8(zraum)+1.
     ENDIF
     IF coinc EQ 10 THEN BEGIN
      geshe25(zraum)=geshe25(zraum)+1.
      IF teson GE 0.95 AND teson LT 1.05 THEN helhe25(zraum)=helhe25(zraum)+1.
     ENDIF
    ENDIF ELSE BEGIN
                                 ;ring on
     IF coinc EQ 4 THEN BEGIN
      gesp4(zraum)=gesp4(zraum)+1.
      IF teson GE 1.73 AND teson LT 1.92 THEN prop4(zraum)=prop4(zraum)+1.
      ENDIF
     IF coinc EQ 5 THEN BEGIN
      gesp8(zraum)=gesp8(zraum)+1.
      IF teson GE 1.73 AND teson LT 1.92 THEN prop8(zraum)=prop8(zraum)+1.
     ENDIF
     IF coinc EQ 6 THEN BEGIN
      gesp25(zraum)=gesp25(zraum)+1.
      IF teson GE 1.65 AND teson LT 1.94 THEN prop25(zraum)=prop25(zraum)+1.
     ENDIF
     IF coinc EQ 8 THEN BEGIN
      geshe4(zraum)=geshe4(zraum)+1.
      IF teson GE 0.89 AND teson LT 1.08 THEN helhe4(zraum)=helhe4(zraum)+1.
     ENDIF
     IF coinc EQ 9 THEN BEGIN
      geshe8(zraum)=geshe8(zraum)+1.
      IF teson GE 0.91 AND teson LT 1.08 THEN helhe8(zraum)=helhe8(zraum)+1.
     ENDIF
     IF coinc EQ 10 THEN BEGIN
      geshe25(zraum)=geshe25(zraum)+1.
      IF teson GE 0.91 AND teson LT 1.08 THEN helhe25(zraum)=helhe25(zraum)+1.
     ENDIF
    ENDELSE
   ENDIF
  ENDIF
  IF zraum GT 1440.*tage-1. THEN a=1
 ENDWHILE
 goto, pdone
...
;print,'phx 1'
FOR zz=300,310 DO BEGIN
 ;print,'el150 ',ele150(zz),' el300 ',ele300(zz)
ENDFOR
FOR a=0.,1440.*tage-1. DO BEGIN
 IF gese150(a) EQ 0. THEN gese150(a)=1.
 IF gese300(a) EQ 0. THEN gese300(a)=1.
 IF gese1300(a) EQ 0. THEN gese1300(a)=1.
 IF gesp4(a) EQ 0. THEN gesp4(a)=1.
 IF gesp8(a) EQ 0. THEN gesp8(a)=1.
 IF gesp25(a) EQ 0. THEN gesp25(a)=1.
 IF geshe4(a) EQ 0. THEN geshe4(a)=1.
 IF geshe8(a) EQ 0. THEN geshe8(a)=1.
 IF geshe25(a) EQ 0. THEN geshe25(a)=1.
 ...
 p4(a)=p4(a)/5.1413/3.5*prop4(a)/gesp4(a)
 p8(a)=p8(a)/5.1413/17.2*prop8(a)/gesp8(a)
 ...
 intc(a)=intc(a)/4.5211    ;new: 7.84
;critical issue: particle species
;correction is done for normal incidence, but normalization is applied
;for all incidences 
endfor
print,'phx 2'
FOR zz=300,310 DO BEGIN
 ;print,'150 ',e150(zz),' 300 ',e300(zz)
ENDFOR
name='/Users/aposner/Documents/Kiel_data/archived/hires/eflr_n2002.asc'   ;*****
openw,unit,name,/variable,/get_lun
printf,unit,yn,e150,e300,e1300,p4,p8,p25,h4,h8,h25,intc
free_lun,unit
...
name='/Users/aposner/Documents/Kiel_data/archived/hires/epherr_nn2002.asc'   ;*****
openw,unit,name,/variable,/get_lun
printf,unit,yn,ele150,ele300,ele1300,prop4,prop8,prop25,helhe4,helhe8,helhe25
free_lun,unit
;print,helhe4
print,'Check:'
print,'yn',max(yn)
print,'ele150',max(ele150)
print,'ele300',max(ele300)
print,'ele1300',max(ele1300)
print,'prop4',max(prop4)
print,'prop8',max(prop8)
print,'prop25',max(prop25)
print,'helhe4',max(helhe4)
print,'helhe8',max(helhe8)
print,'helhe25',max(helhe25)
